input :: [(Int, Int, Char, [Char])]
input = [(6, 7, 'z', "dqzzzjbzz"),
...
filterLetter :: Eq a => a -> [a] -> [a]
filterLetter letter xs = [l | l <- xs, l == letter]

isCorrect :: Eq a => Int -> Int -> a -> [a] -> Bool
isCorrect min max letter xs = do 
    let len = length (filterLetter letter xs)
    len `elem` [min..max]

tobogganIsCorrect :: Eq a => Int -> Int -> a -> [a] -> Bool
tobogganIsCorrect index1 index2 letter xs = do
    let firstLetter = xs !! (index1 - 1)
    let secondLetter = xs !! (index2 - 1)
    (firstLetter == letter || secondLetter == letter) && firstLetter /= secondLetter

result :: (Num a1, Eq a2) => [(Int, Int, a2, [a2])] -> a1
result xs = sum [1 | (min, max, letter, xxs) <- xs, isCorrect min max letter xxs]

tobogganResult :: (Num a1, Eq a2) => [(Int, Int, a2, [a2])] -> a1
tobogganResult xs = sum [1 | (index1, index2, letter, xxs) <- xs, tobogganIsCorrect index1 index2 letter xxs]